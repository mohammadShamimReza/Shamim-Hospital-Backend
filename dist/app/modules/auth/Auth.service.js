...
const forgotPass = async ({ email, role }) => {
    ...
    const passResetToken = await jwtHelpers.createResetToken({ id: isUserExist.id, role: isUserExist.role }, config.jwt.secret, 50);
...
const resetPassword = async (payload) => {
    ...
    const isVarified = await jwtHelpers.verifyToken(token, config.jwt.secret);
